...
In Codex, SSO-backed AWS profiles frequently need local cache access outside the workspace. For `aws sts get-caller-identity`, `aws sso login --use-device-code`, and other user-authorized AWS CLI commands that must read or write `~/.aws/sso/cache`, `~/.aws/cli/cache`, or `~/.aws/login/cache`, use sandbox escalation for the exact AWS command instead of waiting for an `Operation not permitted` cache failure. Never print token or cache file contents.
