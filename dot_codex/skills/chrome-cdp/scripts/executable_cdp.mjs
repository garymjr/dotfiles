#!/usr/bin/env node
// cdp - lightweight Chrome-family DevTools Protocol CLI
...
// the CDP session open. The browser's "Allow debugging" modal fires once per
...
function getWsUrl() {
  ...
  if (!portFile) throw new Error('No DevToolsActivePort found. Enable remote debugging in your Chrome-family browser at chrome://inspect/#remote-debugging, or set CDP_PORT_FILE.');
...
const USAGE = `cdp - lightweight Chrome-family DevTools Protocol CLI (no Puppeteer)
